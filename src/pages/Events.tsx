import { useEffect, useMemo, useState } from "react";
import { useNavigate } from "react-router-dom";
import { useOrg } from "@/contexts/OrgContext";
import { supabase } from "@/integrations/supabase/client";
import PageHeader from "@/components/shared/PageHeader";
import { Skeleton } from "@/components/ui/skeleton";
import { Input } from "@/components/ui/input";
import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from "@/components/ui/select";
import { Activity, Search } from "lucide-react";
import ActivityFeed, { type FeedItem } from "@/components/shared/ActivityFeed";
import { toFeedItem, type RawEvent, type RawLog } from "@/lib/activityMapping";

function getNavPath(type: string, metadata: Record<string, unknown> | null): string | null {
  const projectId = metadata?.project_id as string | null;
  const documentId = metadata?.document_id as string | null;
  switch (type) {
    case "PROJECT_CREATED":
    case "PROJECT_UPDATED":
    case "PROJECT_MEMBER_ADDED":
    case "PROJECT_MEMBER_REMOVED":
      return projectId ? `/projects/${projectId}` : "/projects";
    case "PROJECT_DELETED":
      return "/projects";
    case "WORKSPACE_CREATED":
    case "OWNERSHIP_TRANSFERRED":
      return "/settings";
    case "DOCUMENT_UPLOADED":
      return documentId ? `/documents/${documentId}` : "/documents";
    default:
      return null;
  }
}

const Events = () => {
  const navigate = useNavigate();
  const { selectedOrgId } = useOrg();
  const [events, setEvents] = useState<RawEvent[]>([]);
  const [logs, setLogs] = useState<RawLog[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState("");
  const [statusFilter, setStatusFilter] = useState("all");
  const [typeFilter, setTypeFilter] = useState("all");

  useEffect(() => {
    if (!selectedOrgId) return;
    let cancelled = false;
    (async () => {
      setLoading(true);
      const { data } = await supabase
        .from("events")
        .select("id, type, metadata, created_at")
        .eq("org_id", selectedOrgId)
        .order("created_at", { ascending: false })
        .limit(50);
      const rows = (data as RawEvent[]) ?? [];

      let logRows: RawLog[] = [];
      if (rows.length > 0) {
        const eventIds = rows.map((e) => e.id);
        const { data: lData } = await supabase
          .from("automation_logs")
          .select("id, status, event_id, automation_rules!inner(action_type, org_id)")
          .eq("automation_rules.org_id", selectedOrgId)
          .in("event_id", eventIds);

        logRows = ((lData ?? []) as any[]).map((r) => ({
          id: r.id,
          status: r.status ?? "pending",
          event_id: r.event_id,
          action_type: r.automation_rules?.action_type ?? "—",
        }));
      }

      if (!cancelled) {
        setEvents(rows);
        setLogs(logRows);
        setLoading(false);
      }
    })();
    return () => {
      cancelled = true;
    };
  }, [selectedOrgId]);

  const typeOptions = useMemo(() => Array.from(new Set(events.map((e) => e.type))), [events]);

  const filteredItems = useMemo<FeedItem[]>(() => {
    const q = search.trim().toLowerCase();
    return events
      .filter((evt) => {
        if (typeFilter !== "all" && evt.type !== typeFilter) return false;
        return true;
      })
      .map((evt) => {
        const path = getNavPath(evt.type, evt.metadata);
        return { evt, item: toFeedItem(evt, logs, path ? () => navigate(path) : undefined) };
      })
      .filter(({ item }) => {
        if (statusFilter !== "all" && item.status !== statusFilter) return false;
        if (q) {
          const hay = `${item.title} ${item.entityName ?? ""}`.toLowerCase();
          if (!hay.includes(q)) return false;
        }
        return true;
      })
      .map(({ item }) => item);
  }, [events, logs, search, statusFilter, typeFilter, navigate]);

  return (
    <main className="p-3 sm:p-6">
      <PageHeader title="Events" description="Activity timeline for your workspace" />

      {/* Filters */}
      <div className="mb-5 flex flex-col gap-2 sm:flex-row">
        <div className="relative flex-1">
          <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
          <Input
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            placeholder="Search timeline..."
            className="h-10 rounded-xl bg-card pl-9 shadow-card border-transparent"
          />
        </div>
        <Select value={statusFilter} onValueChange={setStatusFilter}>
          <SelectTrigger className="h-10 w-full rounded-xl bg-card shadow-card border-transparent sm:w-[160px]">
            <SelectValue placeholder="All Statuses" />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">All Statuses</SelectItem>
            <SelectItem value="completed">Completed</SelectItem>
            <SelectItem value="pending">Pending</SelectItem>
            <SelectItem value="running">Running</SelectItem>
            <SelectItem value="failed">Failed</SelectItem>
            <SelectItem value="neutral">No automations</SelectItem>
          </SelectContent>
        </Select>
        <Select value={typeFilter} onValueChange={setTypeFilter}>
          <SelectTrigger className="h-10 w-full rounded-xl bg-card shadow-card border-transparent sm:w-[170px]">
            <SelectValue placeholder="All Events" />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">All Events</SelectItem>
            {typeOptions.map((t) => (
              <SelectItem key={t} value={t}>
                {t.replace(/_/g, " ").toLowerCase().replace(/^\w/, (c) => c.toUpperCase())}
              </SelectItem>
            ))}
          </SelectContent>
        </Select>
      </div>

      {loading ? (
        <div className="space-y-3">
          {Array.from({ length: 5 }).map((_, i) => (
            <div key={i} className="flex items-start gap-4 rounded-xl bg-card px-5 py-4 shadow-card">
              <Skeleton className="h-9 w-9 rounded-xl shrink-0" />
              <div className="flex-1 space-y-2">
                <Skeleton className="h-4 w-64" />
                <Skeleton className="h-3 w-32" />
              </div>
              <Skeleton className="h-6 w-20 rounded-full" />
            </div>
          ))}
        </div>
      ) : filteredItems.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-2xl bg-card py-14 text-center shadow-card">
          <div className="mb-4 flex h-11 w-11 items-center justify-center rounded-full bg-muted">
            <Activity className="h-5 w-5 text-muted-foreground" />
          </div>
          <p className="text-[15px] font-semibold text-foreground">No matching activity</p>
          <p className="mt-1.5 max-w-sm text-sm text-muted-foreground">
            Activity will appear here when events are triggered.
          </p>
        </div>
      ) : (
        <ActivityFeed items={filteredItems} />
      )}
    </main>
  );
};

export default Events;
